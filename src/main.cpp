...
void TextRendering_ShowFramesPerSecond(GLFWwindow* window, float fps);
...
int main(int argc, char* argv[])
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 960, "INF01047 - 278295 - Leandro de Souza Rodrigues", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
...
//    glfwSetScrollCallback(window, ScrollCallback);
    ...
    FramebufferSizeCallback(window, 800, 960); // Forçamos a chamada do callback acima, para definir g_ScreenRatio.
    ...
    ///AQUI A GENTE TÁ CARREGANDO MODELOS
...
//    spheremodel->shapes
    ...
    ComputeNormals(&modeloPaleta);
    BuildTrianglesAndAddToVirtualScene(&modeloPaleta);

    ComputeNormals(&modeloCubo);
    BuildTrianglesAndAddToVirtualScene(&modeloCubo);

    ComputeNormals(&modeloCubo2);
    BuildTrianglesAndAddToVirtualScene(&modeloCubo2);

    ComputeNormals(&modeloCuboQuebrando1);
    BuildTrianglesAndAddToVirtualScene(&modeloCuboQuebrando1);

    ComputeNormals(&modeloCuboQuebrando2);
    BuildTrianglesAndAddToVirtualScene(&modeloCuboQuebrando2);

    ComputeNormals(&modelPlane);
    BuildTrianglesAndAddToVirtualScene(&modelPlane);

    ///ATÉ AQUI A GENTE TÁ CARREGANDO MODELOS
    ...
    initClock();
    initInput(window);
    //BaseCollidingObj objTstBase(glm::vec3(3,5,0),&modeloCubo2,BB_CUBICA,false);
    //GameObjTst2 objTst2(glm::vec3(-3,0,0),&spheremodel,BB_ESFERICA);
    Parede objTstParede(glm::vec3(0,26,0),&modeloCubo,1,21);
    Parede objTstParede2(glm::vec3(-21.5,0,0),&modeloCubo,27,1);
    Parede objTstParede3(glm::vec3(21.5,0,0),&modeloCubo,27,1);
    Paleta objTstPaleta(glm::vec3(0,-19.5,0),PARA_CIMA,&modeloPaleta);
...
//        glUniformMatrix4fv(model_uniform, 1 , GL_FALSE , glm::value_ptr(objTstBase.model));
...
//        glUniformMatrix4fv(model_uniform, 1 , GL_FALSE , glm::value_ptr(objTst2.model));
//        glUniform1i(object_id_uniform, SPHERE);
//        glUniform1f(alpha_uniform, objTst2.alphaFactor);
//        if(objTst2.active) DrawVirtualObject(objTst2.shapeName);

        glUniformMatrix4fv(model_uniform, 1 , GL_FALSE , glm::value_ptr(objTstBolinha.model));
        glUniform1i(object_id_uniform, SPHERE);
        glUniform1f(alpha_uniform, objTstBolinha.alphaFactor);
        if(objTstBolinha.active)  DrawVirtualObject(objTstBolinha.shapeName);
        ...
        for (int i = 0; i < COLUNAS_BLOQUINHOS; i++){
            for(int j = 0; j < LINHAS_BLOQUINHOS; j++){
                ...
                if(arrayBloquinhos[i*LINHAS_BLOQUINHOS + j]->active){
                    blocksAlive += 1;
                    DrawVirtualObject(arrayBloquinhos[i*LINHAS_BLOQUINHOS + j]->shapeName);
                }
            }
        }
...
//
        ...
        ///OBJETOS DE TESTE DESENHADOS AQUI EM CIMA
...
//        DrawCube(1);
        ...
        TextRendering_ShowFramesPerSecond(window, getFPS());
        ...
        if (g_actionKB[EXIT][STATE] == JUST_PRESSED)
            glfwSetWindowShouldClose(window, GL_TRUE);
        if (g_actionKB[PERSPECTIVE_PROJ][STATE]  == JUST_PRESSED)
            g_UsePerspectiveProjection = true;
        if (g_actionKB[ORTHO_PROJ][STATE] == JUST_PRESSED)
            g_UsePerspectiveProjection = false;
        if (g_actionKB[RESET_CAMERA][STATE] == JUST_PRESSED){
            g_CameraTheta = CAMERA_THETA_ZERO;
            g_CameraPhi = CAMERA_PHI_ZERO;
            g_CameraDistance = CAMERA_DIST_ZERO;
        }
...
void TextRendering_ShowFramesPerSecond(GLFWwindow* window, float fps)
{
    ...
    if ( ellapsed_seconds > 1.0f )
    {
        ...
        ///
        numchars = snprintf(buffer, 20, "%.2f fps", fps);
        ///
